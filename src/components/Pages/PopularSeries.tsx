...
import { Picture, POPULAR_SERIES_PATHS } from '../../data';
import '../../styles/PicturesPage.scss';
import Spinner from '../Spinner';

function PopularSeries(): JSX.Element {
  const { popularSeries } = useContext(Context);
  const [picTo, setPicTo] = useState<string>(POPULAR_SERIES_PATHS.picTo);
  const [typeTo, setTypeTo] = useState<string>(POPULAR_SERIES_PATHS.typeTo);
  const [searchTo, setSearchTo] = useState<string>(
    POPULAR_SERIES_PATHS.searchTo
  );
  ...
  useEffect(() => {
    if (popularSeries.length > 0) setPictures(popularSeries);
  }, [popularSeries]);
