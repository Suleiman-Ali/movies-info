...
import { Picture, TOP_SERIES_PATHS } from '../../data';
import '../../styles/PicturesPage.scss';
import Spinner from '../Spinner';

function TopSeries(): JSX.Element {
  const { topSeries } = useContext(Context);
  const [picTo, setPicTo] = useState<string>(TOP_SERIES_PATHS.picTo);
  const [typeTo, setTypeTo] = useState<string>(TOP_SERIES_PATHS.typeTo);
  const [searchTo, setSearchTo] = useState<string>(TOP_SERIES_PATHS.searchTo);
  ...
  useEffect(() => {
    if (topSeries.length > 0) setPictures(topSeries);
  }, [topSeries]);
